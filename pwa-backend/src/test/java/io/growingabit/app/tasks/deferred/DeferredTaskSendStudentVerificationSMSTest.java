...
public class DeferredTaskSendStudentVerificationSMSTest extends BaseGaeTest {
  ...
  @Test
  public void keyNull() {
    final DeferredTaskSendStudentVerificationSMS d = new DeferredTaskSendStudentVerificationSMS(null);
    try {
      d.run();
      assert true;
  ...
  public void keyEmpty() {
    final DeferredTaskSendStudentVerificationSMS d = new DeferredTaskSendStudentVerificationSMS("");
    try {
      d.run();
      assert true;
    } catch (final Throwable t) {
  ...
  @Test
  public void notAKey() {
    final DeferredTaskSendStudentVerificationSMS d = new DeferredTaskSendStudentVerificationSMS("foo");
    try {
      d.run();
      assert true;
    } catch (final Throwable t) {
  ...
  public void wrongKey() {
    final DeferredTaskSendStudentVerificationSMS d = new DeferredTaskSendStudentVerificationSMS(Key.create(StudentPhoneSignupStage.class, 1L).toWebSafeString());
    try {
      d.run();
      assert true;
    } catch (final Throwable t) {
  ...
  public void twilioInitThrowException() {
    try {
      ...
      final DeferredTaskSendStudentVerificationSMS task = new DeferredTaskSendStudentVerificationSMS(stage.getWebSafeKey());
      task.run();
